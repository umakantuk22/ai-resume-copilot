...
// 🛰️ SECURE ENTERPRISE-GRADE DYNAMIC CORS ARCHITECTURE
const whitelistedOrigins = [
    "http://127.0.0.1:5500",
    "http://localhost:5500",
    "http://127.0.0.1:5000",
    "http://localhost:5000",
    "https://uk-resume-copilot.vercel.app" // Main custom production domain
];

app.use(cors({
    origin: function (origin, callback) {
        // Allow server-to-server requests or matching system tools (like Postman or health checks)
        if (!origin) return callback(null, true);
        
        // 1. Check if the origin matches our core explicit whitelist array
        if (whitelistedOrigins.includes(origin)) {
            return callback(null, true);
        }
        
        // 2. Safely authorize ANY official dynamic deployment URL generated by your Vercel project account
        if (origin.endsWith(".vercel.app") && origin.includes("umakant-sharma-s-projects")) {
            return callback(null, true);
        }
        
        // If it doesn't match local or official Vercel footprints, reject it immediately for security
        return callback(new Error("Blocked by AI Resume Copilot Centralized Security Matrix"));
    },
    methods: ["GET", "POST"],
    credentials: true
}));
